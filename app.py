...
def _solve_lotto(pool: list[dict], stake: float, target_today: float) -> list[dict]:
    ...
    # No sportsbook cap — Lotto is an analysis view. You're scanning the
    # top 3 legs per game to cherry-pick, not placing the full ticket.
    variants = [
        ("Min legs",   min_n,        "Smallest parlay that hits your target."),
        ("Boost",      boost_n,      "Extra cushion — bigger payout, similar legs."),
        ("Max Lotto",  total_games,  f"All {total_games} top-3-per-game legs. Browse & cherry-pick."),
    ]
